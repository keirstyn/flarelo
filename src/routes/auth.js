...
//
// role defaults to technician unless the owner explicitly requests
// 'owner' or 'office'. Any other value (or none) falls back to
// technician rather than erroring — matches the original permissive
...
export async function handleInvite(request, env, { sendEmailFn = sendEmail } = {}) {
  ...
  const role = ['owner', 'office'].includes(body.role) ? body.role : 'technician';
